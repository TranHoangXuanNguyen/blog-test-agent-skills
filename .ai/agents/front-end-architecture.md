# The Ultimate Frontend Development Guide: Principles, Patterns, and Practices
  ...
  - File names for helper/utility scripts, styles, and custom hooks (e.g., `use-auth.ts`, `index.css`). Note: React component files in this project use **PascalCase** (e.g., `Header.tsx`, `About.tsx`) to match the existing codebase.
  ...
  ### Server Components First (Next.js Projects Only)
  
  - *Note: This project is a React SPA running on Vite. The Server Components rules below are only applicable if/when migrating to Next.js.*
  ...
  - Implement proper data fetching using Next.js patterns (or standard fetch/hooks for Vite SPA)
