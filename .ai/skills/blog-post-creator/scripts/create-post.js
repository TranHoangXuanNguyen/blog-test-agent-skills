...
const customContent = process.argv[6]; // Optional raw markdown content
...
const targetFile = path.join(process.cwd(), 'src', 'data', 'posts.ts');
...
// 1. Tạo tệp markdown trong src/blogs/ nếu chưa tồn tại
const blogsDir = path.join(process.cwd(), 'src', 'blogs');
if (!fs.existsSync(blogsDir)) {
  fs.mkdirSync(blogsDir, { recursive: true });
}

// Chuẩn hóa tên file bằng cách loại bỏ các ký tự không hợp lệ cho hệ thống tệp tin
...
if (!fs.existsSync(mdFilePath)) {
  const defaultMdContent = customContent || `## ${title}\n\nNội dung đang được cập nhật cho bài viết: ${title}.\n`;
  fs.writeFileSync(mdFilePath, defaultMdContent, 'utf8');
  console.log(`- Đã khởi tạo file markdown tại: src/blogs/${mdFileName}`);
}

let postsContent = fs.readFileSync(targetFile, 'utf8');

// 2. Chèn câu lệnh import vào đầu file posts.ts
const importName = title.replace(/[^a-zA-Z0-9]/g, '') + 'Content';
const importLine = `import ${importName} from '../blogs/${sanitizeFileName(title)}.md?raw';`;

if (!postsContent.includes(importLine)) {
  // Tìm vị trí của "export interface Post" hoặc "export const posts" để chèn lên trước
  const insertIndex = postsContent.indexOf('export interface') !== -1
    ? postsContent.indexOf('export interface')
    : postsContent.indexOf('export const');

  if (insertIndex !== -1) {
    postsContent = postsContent.slice(0, insertIndex) + importLine + '\n' + postsContent.slice(insertIndex);
  } else {
    // Fallback chèn vào cuối imports
    postsContent = importLine + '\n' + postsContent;
  }
}

// 3. Phân tích mảng posts để chèn phần tử mới
const arrayRegex = /(export const posts: Post\[] = \[\s*)([\s\S]*?)(\s*\];)/;
const match = postsContent.match(arrayRegex);

if (!match) {
  console.error('Lỗi: Không tìm thấy mảng posts trong file src/data/posts.ts');
  process.exit(1);
}
...
// Định dạng ngày hôm nay (Ví dụ: 28 Th5, 2026)
const dateObj = new Date();
const months = ["Th1", "Th2", "Th3", "Th4", "Th5", "Th6", "Th7", "Th8", "Th9", "Th10", "Th11", "Th12"];
const formattedDate = `${dateObj.getDate()} ${months[dateObj.getMonth()]}, ${dateObj.getFullYear()}`;

// Chọn ảnh mặc định dựa trên category hoặc sử dụng customImageUrl được truyền vào
let imageUrl = process.argv[7] || "https://images.unsplash.com/photo-1498050108023-c5249f4df085?w=800&auto=format&fit=crop&q=60";
if (!process.argv[7]) {
  ...
  } else if (lowerCategory.includes('perf') || lowerCategory.includes('speed') || lowerCategory.includes('tối ưu') || lowerCategory.includes('ai') || lowerCategory.includes('tech')) {
...
const updatedContent = postsContent.replace(arrayRegex, `${prefix}${newArrayBody}\n${suffix}`);
